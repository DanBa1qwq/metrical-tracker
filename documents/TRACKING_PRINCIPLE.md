...
估算单帧耗时: 310迭代 × 13ms ≈ 4秒
总耗时估算: 250帧 × 4秒 ≈ 16-20分钟
```
...
**总结**: Metrical Photometric Tracker 通过密集的逐帧优化，在每帧上预测 FLAME 模型的约 560 个参数（表情、姿态、纹理、光照）和相机参数，最终输出高质量的 3D 面部网格序列。其耗时主要来自于多尺度优化策略和计算密集的可微渲染过程，这是获得高精度跟踪结果的必要代价。
